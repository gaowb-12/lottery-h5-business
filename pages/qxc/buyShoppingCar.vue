<template>
	<view class="box">
		...
		<nav-bar :title="'七星彩'" :back="true"></nav-bar>
						...
						<div class="selected_left">
							<div class="selected_text">
								<div class="selected_num">
									<div class="content" v-for="a in arr.hundredMyriad">
										<p>{{a}}</p>
									</div>
									<span class="vertical" v-if="arr.hundredMyriad!=null">|</span>
									<div class="content" v-for="a in arr.tenMyriad">
										<p>{{a}}</p>
									</div>
									<span class="vertical" v-if="arr.tenMyriad!=null">|</span>
									...
									<div class="content" style="background-color: #007BED;" v-for="c in arr.individual">
										<p>{{c}}</p>
									</div>
	...
	export default {
		...
		methods: {
			...
			calculation(obj) {
				...
				if (data.length > 0) {
					...
					if (!isRepeat) {
						//如果不是重复的继续叠加
						data.unshift(obj)
						uni.setStorageSync("qixing", data)
					}
					//赋值
					this.placeData = data
					//计算总价和总注数
					...
					uni.setStorageSync("qixing", this.placeData)
			...
			random() {
				...
				let arr = this.globalUtil.randomFromZero(10, 6);
				let arr1 = this.globalUtil.randomFromZero(15, 1);
				let obj = {
					uid: uid,
					mode: 0,
					notes: 1,
