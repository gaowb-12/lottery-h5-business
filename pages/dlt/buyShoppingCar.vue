<template>
	<view class="box">
		...
		<nav-bar :title="'大乐透'" :back="true"></nav-bar>
...
<script>
	...
	export default {
		...
		methods: {
			...
			betting() {
				uni.showLoading();
				let data = uni.getStorageSync('dlt');
				if (data.length <= 0) {
					uni.showToast({
						title: '至少选择一注',
						icon: 'none'
			...
			calculation(obj) {
				...
				if (data.length > 0) {
					...
					uni.setStorageSync("dlt", this.placeData)
			...
			random() {
				...
				let numberArr = this.randomFromZero(35, 5);
				let numArr = this.randomFromZero(12, 2);
				let data1 = [];
				//组装排3数据
