<template>
	<view class="wp box">
		...
		<cmd-nav-bar back title="提现" font-color="#fff" background-color="#515ffb" right-text="提现记录"
			@rightText="rightBtn">
		</cmd-nav-bar>
		...
		<view class="header">
			...
			<view class="txt-xh">
				<view class="h3" style="font-size: 50px;" >
					¥{{user.price + user.gold}}
				</view>
				<text>可提现金额(元)</text>
				<text class="s2">?</text>
			</view>
		...
		<view class="boe">
			<view class="m-about">
				...
				<view class="num"  style="margin-top: 10px;">
					<input class="inp" v-model="withdrawalParam.amount" placeholder="请输入金额" type="number" style="font-size: 26px;">
				</view>
				<view class="h3" style="display: flex;justify-content: space-between;align-items: center;">
					...
					<view>
						<span style="color: #515ffb;" @click="see">查看原因</span>
					</view> -->
				</view>
				<view>
				...
				<view style="padding-bottom: 30px;" v-show="zfbShow">
					...
					<u-input placeholder="支付宝账号" v-model="user.zfbNumber" @blur="inputBlur" style="margin: 10px 0;">
						<u-text text="支付宝账号" slot="prefix" margin="0 3px 0 0" type="tips"></u-text>
					</u-input>
					
					<u-input placeholder="请输入姓名" v-model="user.name" @blur="inputBlur">
		...
		<view class="main">
			<view class="txt-xh1">
				<view class="tit">
					...
					<view class="info">
						<text>1.为了优化店主提现转账速度,每天限制提现一次</text>
					</view>
					<view class="info">
						<text>2.如有紧急情况，请找店主后台扣款，线下处理提现</text>
					</view>
					<view class="info">
						<text>3.本平台为店主的数字化管理工具，提现需要店主手动打款，用户资金是由店主托管和监管，如出现资金问题请及时联系店主处理与平台无关</text>
					</view>
					<view class="info">
						<text>4.如有联系不上店主需要帮助的，请在首页右上角点击投诉并描述具体情况和诉求，平台客服会协助您处理</text>
					</view>
					<!-- <view class="desc">
						<text>委托出让</text>
						<text>继续认养猪崽</text>
