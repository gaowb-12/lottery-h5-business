<template>
	<view class="box">
		...
		<nav-bar :title="'他的客户'" :back="true"></nav-bar>
		<view>
		...
		<view class="invitation">
			<p v-if="userShow" style="font-size: 14px;margin-bottom: 20rpx;">共邀请：<span class="font">{{registerUserList.length}}</span>个客户</p>
			<p v-else style="font-size: 14px;margin-bottom: 20rpx;">总投注金额：<span class="font">{{total}}</span>元</p>
		...
		<view class="table" v-if="userShow">
			<uni-table stripe emptyText="暂无更多数据">
				...
				<uni-tr>
					...
					<uni-th align="center" width="60" >流水</uni-th>
				...
				<uni-tr v-for="item in registerUserList" :key="item.id">
					...
					<uni-td align="center" ><span style="color: #515ffb;" @click="mingxi(item)">明细</span></uni-td>
	...
	export default {
		...
		methods: {
			...
			shopRecharge(){
				...
				console.log('_rechargeId ：' + _rechargeId)
