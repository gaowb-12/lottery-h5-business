	...
	<div class="football-confirm padding-bottom-90 box">
		...
		<div>
			<div class="padding" style="display: flex;justify-content: space-around;align-items: center;">
				<u-button @click="continues" style="width: 40%;" color="#515ffb" text="添加赛事"></u-button>
				<u-button @click="documentary" style="width: 40%;" color="#515ffb" text="发起跟单"></u-button>
			</div>
			<div class="padding">
							...
							<u-row class="body" customStyle="margin-bottom: 10px" justify="center" align="center"
								...
								<u-col span="12" textAlign="center" justify="center" align="center">
									<u-button style="border-radius: 5px;" color='#F2F2F2' class="font">
										<span style="color: #515ffb;" v-for="(notLet,index) in item.notLetOddsList"
											v-if="notLet.active">
											{{notLet.describe}}({{notLet.odds}}),
										</span>

										<span style="color: #515ffb;" v-for="(lets,index) in item.letOddsList"
											v-if="lets.active">
											让{{lets.describe}}({{lets.odds}}),
										</span>

										<span style="color: #515ffb;" v-for="(goal,index) in item.goalOddsList"
											v-if="goal.active">
											{{goal.describe}}({{goal.odds}}),
										</span>

										<span style="color: #515ffb;" v-for="(half,index) in item.halfWholeOddsList"
											v-if="half.active">
											{{half.describe}}({{half.odds}}),
										</span>

										<span style="color: #515ffb;" v-for="(score,index) in item.scoreOddsList"
											v-if="score.active">
			...
			<div class="buyFooter_wrap">
				...
				<div class="buyFooter_count">
					<view
						...
						投<u-number-box :value="times" iconStyle="color: #515ffb" integer inputWidth="70"
							@change="numberChange">
						</u-number-box>倍
					</view>
		...
		methods: {
			...
			init(itemList, flag) {
				...
				if (itemList.length > 0) {
					...
					var s2 = false;
					for (var i = 0; i < this.selectItem.length; i++) {
						if (this.selectItem[i].halfWholeOddsList.length != 0 || this.selectItem[i].scoreOddsList.length !=
							0) {
							s1 = true;
						} else if (this.selectItem[i].goalOddsList.length != 0) {
							s2 = true;
						}
					}
					...
					this.calculationParam.footballMatchList = this.selectItem;
