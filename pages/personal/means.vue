<template>
	<view class="box">
			...
			<u-cell-group>
				...
				<u-cell clickable center title="微信号">
					<template slot="value">
						<u-input border="none" v-model="user.wechat" inputAlign="right"></u-input>
						<image src="/static/images/personal/my_40_40.jpg" mode="scaleToFill" border="0" class="my_57" />
					</template>
				</u-cell>
				...
				<a @click="routeBtn" v-show="user.isReal=='0'">
					<u-cell clickable center title="实名认证">
						<template slot="value">
							...
							<image src="/static/images/personal/my_40_40.jpg" mode="scaleToFill" border="0"
								class="my_57" />
						</template>
					</u-cell>
				</a>
				...
				<u-datetime-picker @close="show=false" closeOnClickOverlay :minDate="-2209017600000"
					@cancel="show=false" :show="show" v-model="defaultDate" mode="date" @confirm="confirm">
				</u-datetime-picker>
			</u-cell-group>
			<view style="width: 90%;margin:0 auto;margin-top: 20px;">
				<u-button @click="sava" color="#515ffb" customStyle="fontSize:16px">
					提交
				</u-button>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			sava() {
				//如果修改了头像就上传到S3
				if (this.user.avatar.startsWith("blob:")) {
					//文件上传
					uni.uploadFile({
