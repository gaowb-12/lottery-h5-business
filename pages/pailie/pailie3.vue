<template>
	<view class="box">
		...
		<cmd-nav-bar back title="排列3" font-color="#fff" background-color="#515ffb" right-text="排列3开奖"
			@rightText="rightBtn">
		</cmd-nav-bar>
		<div style="height:100%">
			<u-subsection style="margin-top: 2px;height: 40px;" fontSize="16" :list="list" mode="button"
				:current="current" activeColor="#515ffb" @change="change3"></u-subsection>
			...
			<view v-if="current==0">
				<u-sticky bgColor="#fff"
					...
					<u-tabs :list="tabs" lineColor="#515ffb"
				...
				<div class="fc" v-if="zindex==0">
					<view
						...
						<u-checkbox-group @change="checkChange" size="15" shape="square" placement="column"
							...
							<u-checkbox labelSize="14" activeColor="#515ffb" :label="'显示遗漏'">
							</u-checkbox>
						</u-checkbox-group>
					</view>
					<ul>
			...
			<view v-if="current==1">
				<u-sticky bgColor="#fff"
					style="margin-bottom: 30px;display: flex;justify-content: center;align-items: center;">
					<u-tabs :list="tabs1" lineColor="#515ffb"
						:activeStyle="{color: '#515ffb',fontWeight: 'bold',transform: 'scale(1.05)'}"
					...
					<ul>
						...
						<li @click="check(3,1,index)" v-for="(item,index) in san" :class="item.active?'active':''">
							...
							<u-checkbox-group shape="square" @change="checkboxChange($event,index,1)"
								...
								<u-checkbox activeColor="#515ffb" :name="item.isGallbladder"
...
<style scoped>
	...
	.fc_tab span.active {
		border-bottom: 1px solid #515ffb;
		color: #515ffb;
	}
